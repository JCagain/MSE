...
- [ ] **Step 3b: Manually append NODE_PORT array to topology.h**

The Mega connects directly to two ESP32 nodes (no gateway relay). Append the following block
to `mega/topology.h` after the generated arrays:
...
**Architecture:** The Mega connects directly to two ESP32 nodes — no gateway relay.
Each node has its own UART connection: Serial1 (pins TX1=18, RX1=19) and Serial2 (pins TX2=16, RX2=17).
`NODE_PORT[i]` in `topology.h` maps each node index to its serial port (1, 2, or 0 = not wired).
...
git commit -m "feat(mega): serial_mesh module — dual UART direct connection, path_push per port"
```
...
```
ESP32 node A TX → Mega RX1 (pin 19)
ESP32 node A RX ← Mega TX1 (pin 18)
ESP32 node B TX → Mega RX2 (pin 17)
ESP32 node B RX ← Mega TX2 (pin 16)
Common GND between all three boards
Mega USB        → Laptop
```
...
Once either ESP32 node sends its first `routing` packet, the Mega should:
1. Update `node_states`
2. Run Dijkstra after `BROADCAST_INTERVAL_MS`
3. Send `path_push` to each directly-connected ESP32 node via its assigned serial port
